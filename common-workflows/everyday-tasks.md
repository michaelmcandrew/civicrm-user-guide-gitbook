...
click “Save”), or sending one to multiple contacts through the
...
Contribution Receipts" from the actions drop-down menu. You will be
given the option to print a PDF letter or send an email. Either way, the
receipt date and time will change for each of the contributions
accordingly.

To find out how you can change the format of receipts, see the chapter
"Contributions" > "Set-Up".

![](/images/CiviCRM_contributions_sendreceipts.png) 

### Thank you letters 

Later, your organisation may wish to send custom thank-you letters to a
group of donors. The first step would be to search and filter your
contributions until you have a target list of recipients. Go to
**Contributions** > **Find Contributions** and enter your search
criteria, which we recommend includes:

-   Thank-you date not set? (a thank you letter has not been sent)
-   Financial Type
-   Contribution Status ("completed" if payment has been confirmed)

In your new results list, select the contributions requiring letters,
choose "Thank-you Letters for Contributions" from the actions drop-down
menu and click "Go". You are now ready to customise your letter. Decide
whether to use a pre-existing template, if the letters should update the
receipt or thank you date against each of the selected contribution
records (under "Record Update Options"), and check the page format
settings. When satisfied, begin to edit the content of your letter.
Remember that you can use tokens to include information relevant to each
contact, ranging from first and last name to the total amount
contributed (see "Tokens and mail merge" for more information). Some of
the key tokens you might want to use for thank you letters are:

-   Postal Greeting or First name/Last name
-   Total amount (the total contributed/donated)
-   Net amount (total contributed minus fee)
-   Receive date (when the donation was received)
-   Campaign (the campaign the contact donated under)
-   Source (what the donor contributed to)

![](/images/CiviCRM_thankyouletters.PNG) 

Once your letter is composed, choose whether to add it as a new template
(or update an existing template). Then click "Make PDF Letters".
